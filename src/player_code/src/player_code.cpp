...
State PlayerCode::update(State state) {
    ...
    ++num_turn;
    uint64_t num_state_bots = state.bots.size(), num_flags = state.flag_offsets.size();
    ...
    // Assinging another quarter to move ahead of the flag locations and create towers
    if(num_turn % 100 == 0){
        for(; bot_count < num_state_bots / 2; ++bot_count){
            auto &bot = state.bots[bot_count];
            auto flag_position = state.flag_offsets[(flag_count++) % num_flags];
            auto nearest_buildable_position = findNearestFreePosition(state, flag_position);
    ...
    if(num_turn % 100 == 0){
        for(; bot_count < num_state_bots; ++bot_count){
            ...
            auto nearest_buildable_position = findNearestFreePosition(state, enemy_base);
            bot.transform(nearest_buildable_position);
        }
    }else{
    ...
    for(uint64_t tower_index = 0; tower_index < tower_count; ++tower_index){
        ...
        for(int64_t enemy_bot_index = 0; enemy_bot_index < state.enemy_bots.size(); ++enemy_bot_index){
        ...
        for(int64_t enemy_tower_index = 0; enemy_tower_index < state.enemy_towers.size(); ++enemy_tower_index){
