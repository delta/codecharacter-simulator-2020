...
private:
  ...
  /**
   * Helper function to validate blast request by an actor and make command_taker call
   * BlastBot internally
   */
...
public:
  ...
  CommandGiver(std::unique_ptr<ICommandTaker> state, std::unique_ptr<logger::ILogger> logger);
